...
- [ ] Install `ufw (uncomplicated firewall)` to avoid exposing ports, (write up) by default it denies all incoming request and allow all outgoing requests, we need to allow the following ports:
      - The new SSH port: `sudo ufw allow <ssh-port>/tcp`
      - HTTP & HTTPS ports: `sudo ufw allow http` & `sudo ufw allow https`
