...
and then run *sudo service ssh reload* to apply the new changes, then from my local machine i could ssh into my vps and login right away, but that was not the case because in the sshd_config file there's a line that includes all the configs files from */etc/ssh/sshd_config.d/.config* that overrides the main configurations, by default, Ubuntu installations add a config file that sits at */etc/ssh/sshd_config/50-cloud-init.conf* and has this only option:
...
that forces prompting a password when connecting to vps via ssh, i just set it to *no* and it worked as expected.
